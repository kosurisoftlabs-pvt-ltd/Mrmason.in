...
/**
 * Servlet implementation class CementSpecsMassUpload
 */
@WebServlet("/CementSpecsMassUpload")
public class CementSpecsMassUpload extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public CementSpecsMassUpload() {
        super();
...
//			System.out.println("Last row count ::"+sheet.getLastRowNum()+1);
			...
			{
				Cell cellValue = null;
				CementDetails md=new CementDetails();
//				System.out.println("Last Row. "+lastrow);
//				System.out.println("Row No. "+rows);
				try {
					...
					else
					{
						...
						objFormulaEvaluator.evaluate(cellValue);
						md.setCEMENT_TYPE(objDefaultFormat.formatCellValue(cellValue,objFormulaEvaluator));
						cellValue=row.getCell(4);
						objFormulaEvaluator.evaluate(cellValue);
						md.setMASS(objDefaultFormat.formatCellValue(cellValue,objFormulaEvaluator));
						...
						objFormulaEvaluator.evaluate(cellValue);
						md.setDISPLAY_TYPE(objDefaultFormat.formatCellValue(cellValue,objFormulaEvaluator));
						cellValue=row.getCell(14);
						objFormulaEvaluator.evaluate(cellValue);
						md.setDISPLAY_TYPE(objDefaultFormat.formatCellValue(cellValue,objFormulaEvaluator));
						cellValue=row.getCell(16);
						objFormulaEvaluator.evaluate(cellValue);
						md.setDESCRIPTION(objDefaultFormat.formatCellValue(cellValue,objFormulaEvaluator));
//						System.out.println("PaintMaterials :: "+md);
						
						InputStream i1s =context.getResourceAsStream("/images/ServiceImage.png");
						md.setCEMENT_IMAGE(i1s);
...
//			System.out.println("Row :: "+rows);
			...
			//item.write(f);
			//upload to database
			al=new ProductsService().addBulkCementSpecs(al);
			}
			}
