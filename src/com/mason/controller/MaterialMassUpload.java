...
import com.mason.bean.PaintMaterialDetails;
import com.mason.bean.PaintsDetails;
import com.mason.service.ProductsService;
import com.mason.utils.PaintsTitles;

/**
 * Servlet implementation class MaterialMassUpload
 */
@WebServlet("/MaterialMassUpload")
public class MaterialMassUpload extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MaterialMassUpload() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			else
			{
			    ...
			    InputStream is = context.getResourceAsStream("/images/ServiceImage.png");
				try
				{
...
//				System.out.println("Row No. "+rows);
				try {
					...
					else
					{
						...
						objFormulaEvaluator.evaluate(cellValue);
						md.setPRICE(objDefaultFormat.formatCellValue(cellValue,objFormulaEvaluator));
...
//			System.out.println("Row :: "+rows);
			...
			//item.write(f);
			//upload to database
			al=new ProductsService().addBulkPaints(al);
			}
			}
		...
		getServletContext().getRequestDispatcher("/UploadedBProducts.jsp").forward(request, response);		
	}
