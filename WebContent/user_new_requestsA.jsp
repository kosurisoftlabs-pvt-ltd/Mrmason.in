   ...
   <body>
        <!-- Begin page -->
            ...
            <jsp:include page="./dashboardSidebar.jsp"></jsp:include>
			  ...
			  <!--=========== form section =============-->
						<div class="col-xl-12 col-sm-12">
						 <span class="bo">User Requests</span>
						<form action="./GetAllRequests" name="form" method="post" onsubmit="return validation()">
						   <div class="form-group col-md-12 col-sm-12"></div>
						   <div class="row">
						   <%
						   String servicetype="",reqDate="",status="";
						   try
						   {
							   ...
							   {
								   reqDate=request.getParameter("status");
							   }
						   ...
						   ArrayList<UserServices> al= new ServicesService().getAllServices();
						   %>
							...
							<select class="form1" id="service" name="status">
								<option value="">All</option>
								<%if(status.equals("R")){ %>
								<option selected="selected" value="R">Request</option>
								<%}else{ %>
								<option value="R">Request</option>
								<%} %>
								<%if(status.equals("I")){ %>
								<option selected="selected" value="I">inProgress</option>
								<%}else{ %>
								<option  value="I">inProgress</option>
								<%} %>
								<%if(status.equals("C")){ %>
								<option selected="selected" value="C">Completed</option>
								<%}else{ %>
                    ...
                    <table class="col-md-12 table-bordered table-striped table-condensed cf">
        	              ...
        	              <table>
							...
							<c:forEach begin="1" end="${numofpages}" var="i">
									<td><a href="./GetAllRequests?page=${i}">${i}</a></td>
							</c:forEach>
							</tr>
						  </table>
					</div>
